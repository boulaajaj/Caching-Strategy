...
namespace Amibou.Infrastructure.Caching.Caches
{
    public class DiskCache : CacheBase
    {
        ...
        private static string GetFileName(string cacheKey) => CleanFileName(cacheKey + ".cache");

        private string GetFileNameSearchPattern(string cacheKey) => GetFileName(cacheKey) + ".*";
